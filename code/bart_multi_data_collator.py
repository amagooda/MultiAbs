...
@dataclass
class DataCollatorWithPaddingClassification_word:
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        batch = self.tokenizer.pad(_features, padding=self.padding, max_length=self.max_length, pad_to_multiple_of=self.pad_to_multiple_of, return_tensors="pt",)
        # batch['labels'] = torch.tensor([[int(y) for y in x['labels']] for x in features])
        labels = [[int(y) for y in x['labels']] for x in features]
        max_labels_len = max([len(x) for x in labels])
        for idx, label_arr in enumerate(labels):
            labels[idx] = labels[idx] + ([-100] * (max_labels_len - len(label_arr)))
